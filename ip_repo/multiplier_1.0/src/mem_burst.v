...
begin
  ...
  always @(posedge mem_clk or posedge rst) begin
    if (rst) begin
      app_wdf_wren_r <= 1'b0;
    end else if (app_wdf_rdy) app_wdf_wren_r <= wr_burst_data_req;
  end
        ...
        MEM_READ: begin
          if (app_rdy) begin
            ...
            if (rd_addr_cnt == rd_burst_len - 1) begin
              ...
              rd_addr_cnt <= 0;
              app_en_r <= 1'b0;
            end else rd_addr_cnt <= rd_addr_cnt + 1;
          end
        ...
        MEM_WRITE_WAIT: begin
          if (app_rdy) begin
            ...
            if (wr_addr_cnt == wr_burst_len - 1) begin
              app_wdf_end_r <= 1'b0;
              app_en_r <= 1'b0;
              if (app_wdf_rdy) state <= WRITE_END;
            end else begin
              wr_addr_cnt <= wr_addr_cnt + 1;
            end
          end else if (~app_en_r & app_wdf_rdy) state <= WRITE_END;
